...
import { IPrescoring, Term } from '../../../../types/interface';
...
const Prescoring = observer(({tab, setTab}: IProps) => {

  const {
    rootStore: { applicationArray, buttonText },
  } = useStore();

  const [loader, setLoader] = useState<boolean>(false);
  const [amountValue, setAmountValue] = useState<string>('15000');
    
  const methods = useForm<IPrescoring>({
    resolver: yupResolver(schemaInitialStep),
  });

  useEffect(() => {
    if ( tab === 0 ) {
      buttonText.setbuttonText('Apply for card');
    }
  }, [tab]);
      
  const onSubmit: SubmitHandler<IPrescoring> = (
    {
      amount, 
      lastName, 
      firstName, 
    ...
    const data = {
      ...
      email,
      birthdate: transformDate(new Date(birthdate)),
      passportSeries,
      passportNumber,
    };
    const getResource = async (url: string, data: {}) => {;
      await applicationArray.fetchApplicationArray(url, data);
      setLocalStorage('PrescoringStep', {PrescoringStep: 1});
      setTab(1);
      setLoader(false);
      setLocalStorage('loanOffers', {loanOffers: applicationArray.getApplicationArray});
    }; 
    setLoader(true);
